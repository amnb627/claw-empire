...
describe("active_cli_processes table", () => {
  ...
  beforeEach(() => {
    const tmpPath = path.join(os.tmpdir(), `claw-pid-${Date.now()}-${Math.random().toString(36).slice(2)}.sqlite`);
  ...
  it("active_cli_processes table exists after applyBaseSchema", () => {
    const row = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='active_cli_processes'")
  ...
  it("idx_active_cli_processes_pid index exists", () => {
    const row = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_active_cli_processes_pid'")
      .get() as { name: string } | undefined;
    expect(row?.name).toBe("idx_active_cli_processes_pid");
  });

  it("insert and delete a row works correctly", () => {
    db.prepare("INSERT INTO active_cli_processes (task_id, pid, provider, worktree_path) VALUES (?, ?, ?, ?)").run(
      "task-001",
      12345,
      "claude",
      "/some/path",
    ...
    const inserted = db.prepare("SELECT * FROM active_cli_processes WHERE task_id = ?").get("task-001") as
      | { task_id: string; pid: number; provider: string }
      | undefined;
    ...
    const deleted = db.prepare("SELECT * FROM active_cli_processes WHERE task_id = ?").get("task-001");
    expect(deleted).toBeUndefined();
  });

  it("INSERT OR REPLACE upserts correctly", () => {
    db.prepare("INSERT OR REPLACE INTO active_cli_processes (task_id, pid, provider) VALUES (?, ?, ?)").run(
      "task-002",
      11111,
      "codex",
    );
    ...
    const row = db.prepare("SELECT pid FROM active_cli_processes WHERE task_id = ?").get("task-002") as
      | { pid: number }
      | undefined;
...
describe("reconcileOrphanedProcesses", () => {
  ...
  beforeEach(() => {
    const tmpPath = path.join(os.tmpdir(), `claw-recovery-${Date.now()}-${Math.random().toString(36).slice(2)}.sqlite`);
    db = new DatabaseSync(tmpPath);
  ...
  it("returns immediately when no orphaned processes exist", async () => {
    ...
    const count = db.prepare("SELECT COUNT(*) AS c FROM active_cli_processes").get() as { c: number };
  ...
  it("resets in_progress tasks to inbox status for orphaned entries", async () => {
    ...
    const task = db.prepare("SELECT status FROM tasks WHERE id = ?").get("task-rp-1") as { status: string } | undefined;
  ...
  it("does not touch tasks that are not in_progress", async () => {
    ...
    const task = db.prepare("SELECT status FROM tasks WHERE id = ?").get("task-done-1") as
      | { status: string }
      | undefined;
