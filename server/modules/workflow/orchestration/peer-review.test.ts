...
function seedPack(db: DatabaseSync, key: string, qaRules: Record<string, unknown>): void {
  db.prepare("INSERT INTO workflow_packs (key, qa_rules_json) VALUES (?, ?)").run(key, JSON.stringify(qaRules));
}
...
function makeOpts(db: DatabaseSync, overrides: Partial<PeerReviewOptions> = {}): PeerReviewOptions {
  return {
    taskId: "task-001",
...
`.trim();
  ...
  it("records a meeting_minutes row with type peer_review", () => {
    ...
    try {
      ...
      const row = db.prepare("SELECT meeting_type FROM meeting_minutes WHERE task_id = 'task-001'").get() as
        | { meeting_type: string }
        | undefined;
  ...
  it("records a meeting_minute_entries row with the feedback", () => {
    ...
    try {
      ...
      const entry = db.prepare("SELECT content FROM meeting_minute_entries WHERE meeting_id = ?").get(rv.meetingId) as
        | { content: string }
        | undefined;
  ...
  it("adds to review_revision_history when peer review fails", () => {
    ...
    try {
      ...
      const history = db.prepare("SELECT * FROM review_revision_history WHERE task_id = 'task-fail'").all() as Array<{
        raw_note: string;
      }>;
  ...
  it("does NOT add to revision history when peer review passes", () => {
    ...
    try {
      ...
      const history = db.prepare("SELECT * FROM review_revision_history WHERE task_id = 'task-pass'").all();
