...
export async function reconcileOrphanedProcesses(db: DatabaseSync): Promise<void> {
  const rows = db.prepare("SELECT task_id, pid, worktree_path FROM active_cli_processes").all() as Array<{
    task_id: string;
    pid: number;
  ...
  for (const row of rows) {
    ...
    try {
      db.prepare(`UPDATE tasks SET status = 'inbox', updated_at = ? WHERE id = ? AND status = 'in_progress'`).run(
        Date.now(),
        row.task_id,
      );
    } catch {
