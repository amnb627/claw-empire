...
export function registerScheduleRoutes(
  ...
  app.get("/api/schedules", (_req, res) => {
    const rows = db.prepare(`SELECT * FROM task_schedules ORDER BY next_trigger_at ASC`).all() as ScheduleRow[];
  ...
  app.post("/api/schedules", (req, res) => {
    ...
    const workflowMetaJson =
      body.workflow_meta_json != null
        ...
        : null;
    const priority = typeof body.priority === "number" ? Math.round(body.priority) : 0;
  ...
  app.put("/api/schedules/:id", (req, res) => {
    const { id } = req.params;
    const existing = db.prepare(`SELECT id FROM task_schedules WHERE id = ?`).get(id) as { id: string } | undefined;
    ...
    if ("workflow_meta_json" in body) {
      ...
      params.push(wmj == null ? null : typeof wmj === "string" ? wmj : JSON.stringify(wmj));
    }
    ...
    if ("enabled" in body) {
      const v = body.enabled === false || body.enabled === 0 || String(body.enabled) === "0" ? 0 : 1;
  ...
  app.post("/api/schedules/:id/trigger", (req, res) => {
    ...
    const now = nowMs();
    db.prepare(`UPDATE task_schedules SET next_trigger_at = ?, enabled = 1 WHERE id = ?`).run(now - 1, id);
