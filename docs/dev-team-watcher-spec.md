    ...
    runYoloDecisionAutopilot({
      ...
      shouldSkipItem: (item) => {
        ...
        if (item.kind === "review_round_pick" && item.task_id) {
          const row = db.prepare("SELECT workflow_pack_key FROM tasks WHERE id = ? LIMIT 1").get(item.task_id);
...
type DecisionInboxKind = "agent_request" | "project_review_ready" | "task_timeout_resume" | "review_round_pick";
