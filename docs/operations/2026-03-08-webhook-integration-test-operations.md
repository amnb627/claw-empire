...
export async function notifyTestFailure(testId: string, testName: string, error: string): Promise<void> {
  await sendSlackNotification({
    text: `🔴 テスト失敗: ${testName}`,
    blocks: [
      {
...
export async function notifyCriticalFailure(testId: string, testName: string, error: string): Promise<void> {
