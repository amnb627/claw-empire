...
export function getWebhookTLSConfig(env: "production" | "test"): https.ServerOptions {
  const baseConfig: https.ServerOptions = {
    key: readFileSync(
      env === "production" ? "/etc/ssl/private/claw-empire-webhook.key" : "./certs/test-webhook-key.pem",
    ),
    cert: readFileSync(
      env === "production" ? "/etc/ssl/certs/claw-empire-webhook.crt" : "./certs/test-webhook-cert.pem",
    ),
...
// TLSバージョンチェックミドルウェア
export function tlsVersionCheckMiddleware(req: any, res: any, next: any) {
...
export class IPWhitelistManager {
  ...
  constructor(environment: "production" | "test") {
    this.configPath = path.join(__dirname, `../../config/ip-whitelist-${environment}.json`);
...
// 本番用IP制限ミドルウェア
export function createIPRestrictionMiddleware(whitelistManager: IPWhitelistManager) {
...
// 設定ファクトリー
export function createRateLimitConfig(environment: "production" | "test"): RateLimitConfig {
  return {
...
export class WebhookSIEMFormatter {
  ...
  formatSecurityEvent(event: { type: string; source_ip: string; details: Record<string, unknown> }): SIEMEvent {
    return {
  ...
  toCEF(event: SIEMEvent): string {
    ...
    return (
      `CEF:0|ClawEmpire|Webhook|1.0|${event.event_type}|${event.event_type}|${severityMap[event.severity]}|` +
...
export class SIEMForwarder {
  ...
  private sendSyslog(event: SIEMEvent): void {
    ...
    const message = this.config.format === "cef" ? this.formatter.toCEF(event) : this.formatter.toJSON(event);

    const syslogMessage = `<${this.getSyslogSeverity(event.severity)}>${message}`;

    try {
      this.socket.send(syslogMessage, this.config.port, this.config.host, (err) => {
        if (err) {
